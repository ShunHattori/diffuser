...
def test_dataset_attributes(env_name):
    ...
    num_render_path = 1000
    for i in range(num_render_path):
        # visualize the path
        plt.scatter(dataset["observations"][timeout_indices[i] : timeout_indices[i + 1], 0], dataset["observations"][timeout_indices[i] : timeout_indices[i + 1], 1])
...
def partially_remove_maze2d_paths(dataset):
    ...
    for i in range(len(timeout_indices) - 1):
        ...
        if np.any(distances < remove_judge_radius):
            ...
            keep_indices[start_idx:end_idx] = False
