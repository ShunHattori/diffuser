...
class MuJoCoRenderer:
    ...
    def composite(self, savepath, paths, dim=(1024, 256), **kwargs):
        ...
        render_kwargs = {"trackbodyid": 2, "distance": 10, "lookat": [5, 2, 0.5], "elevation": 0}
        images = []
        for path in paths:
            ## [ H x obs_dim ]
            path = atmost_2d(path)
    ...
    def render_plan(self, savepath, actions, observations_pred, state, fps=30):
        ...
        images_pred = np.stack([self._renders(obs_pred, partial=True) for obs_pred in observations_pred])

        images_real = np.stack([self._renders(obs_real, partial=False) for obs_real in observations_real])
...
class MazeRenderer:
    ...
    def renders(self, observations, conditions=None, title=None):
        ...
        plt.imshow(self._background * 0.5, extent=self._extent, cmap=plt.cm.binary, vmin=0, vmax=1)

        path_length = len(observations)
        colors = plt.cm.jet(np.linspace(0, 1, path_length))
        # plt.scatter(0, 0, c="pink", zorder=30)  # origin 左上
        # plt.scatter(0.5, 0.5, c="pink", zorder=30)  # origin 左上
...
def set_state(env, state):
    ...
    if not state.size == qpos_dim + qvel_dim:
        warnings.warn(f"[ utils/rendering ] Expected state of size {qpos_dim + qvel_dim}, " f"but got state of size {state.size}")
...
def rollouts_from_state(env, state, actions_l):
    rollouts = np.stack([rollout_from_state(env, state, actions) for actions in actions_l])
    return rollouts
