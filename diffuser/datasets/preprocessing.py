...
def arctanh_actions(*args, **kwargs):
    ...
    def _fn(dataset):
        ...
        assert actions.min() >= -1 and actions.max() <= 1, f"applying arctanh to actions in range [{actions.min()}, {actions.max()}]"
...
def maze2d_set_terminals(env):
    ...
    def _fn(dataset):
        ...
        print(f"[ utils/preprocessing ] Segmented {env.name} | {len(path_lengths)} paths | " f"min length: {path_lengths.min()} | max length: {path_lengths.max()}")
        print(f'dataset["timeouts"].shape: {dataset["timeouts"].shape}')
        print(f'dataset["timeouts"].sum(): {dataset["timeouts"].sum()}')

        dataset["timeouts"] = timeouts  # パスの数を減らしてその後の処理を軽量化？ sequence.py のnormalizeで激重
